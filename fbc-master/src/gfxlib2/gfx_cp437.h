/* CP437 <-> Unicode, GENERATED by tools/gen_cp437_table.py -- do not edit.
 *
 * gfxlib2's font is a 256-glyph bitmap indexed by byte, and gfxlib2 declares
 * its charset as CP437 (FB_GFX_GET_CHARSET). Drawing a USTRING therefore
 * means mapping each code unit to its CP437 byte.
 *
 * A TABLE, not the C locale. The existing wstring path converts via
 * fb_wstr_ConvToA, so the same source draws different glyphs depending on the
 * machine's locale. A portable string type cannot behave that way.
 */

#ifndef __FB_GFX_CP437_H__
#define __FB_GFX_CP437_H__

/* CP437 byte -> Unicode codepoint. Every entry is in the BMP, so a uint16
   holds it and it is directly comparable to a USTRING code unit. */
static const unsigned short __fb_gfx_cp437_to_uni[256] = {
