...
{ _ '' class              size  signed intrank remaptype         sizetype             name
	...
	( FB_DATACLASS_STRING , -1, FALSE,  0, FB_DATATYPE_USTRING , -1                 , @"ustring"  ), _  '' size patched to 12/24 in symbDataInit()
	( FB_DATACLASS_STRING ,  2, FALSE,  0, FB_DATATYPE_FIXUSTR , -1                 , @"ustring"  )  _  '' 2 bytes per code unit, on EVERY target
}
...
sub symbDataInit( )
	if( fbIs64bit( ) ) then
		...
		'' FBUSTRING has the same shape as FBSTRING (ptr + 2 ssize_t), so the
		'' same number. src/rtlib/fb_ustring.h carries a compile-time assert
		'' that the C struct really is this size -- the two must not drift.
		symb_dtypeTB(FB_DATATYPE_USTRING ).size = 24
		...
		symb_dtypeTB(FB_DATATYPE_USTRING ).size = 12
...
function typeNeedsDtorCall _
	...
	function = typeHasDtor( dtype, subtype ) or _
		(typeGetDtAndPtrOnly( dtype ) = FB_DATATYPE_STRING) or _
		(typeGetDtAndPtrOnly( dtype ) = FB_DATATYPE_USTRING)
end function
