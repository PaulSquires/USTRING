...
enum FB_DATATYPE
	...
	FB_DATATYPE_POINTER
	FB_DATATYPE_XMMWORD
	'' USTRING: a portable UTF-16 string. Mirrors the STRING/FIXSTR pair --
	'' USTRING is the var-len descriptor, FIXUSTR is USTRING * N.
	'' APPENDED here on purpose, NOT inserted next to STRING/FIXSTR where it
	'' would read better: every table listed above is a POSITIONAL initializer,
	'' so inserting shifts entries silently and the compiler cannot catch it.
	FB_DATATYPE_USTRING
	FB_DATATYPE_FIXUSTR
end enum
...
const FB_DATATYPES = (FB_DATATYPE_FIXUSTR - FB_DATATYPE_VOID) + 1
