...
function symbIsString _
	...
	case FB_DATATYPE_STRING, FB_DATATYPE_FIXSTR, FB_DATATYPE_CHAR, FB_DATATYPE_WCHAR, _
...
function symbGetStrLength( byval sym as FBSYMBOL ptr ) as longint
	...
	case FB_DATATYPE_USTRING
		'' as FB_DATATYPE_STRING: lgt is the descriptor size, not a length
		function = (sym)->lgt
	case FB_DATATYPE_FIXUSTR
		'' as FB_DATATYPE_WCHAR: lgt is bytes, and the declared count INCLUDES
		'' the terminator, so USTRING * N stores N-1 code units of text
		function = ((sym)->lgt \ typeGetSize( FB_DATATYPE_FIXUSTR )) - 1
	end select
end function
...
'' Length in CODE UNITS of a USTRING * N, excluding the terminator.
'' The ustring twin of symbGetWstrLength(); a code unit is always 2 bytes, so
'' unlike the wstring version this divisor never varies by target.
function symbGetUstrLength( byval sym as FBSYMBOL ptr ) as longint
	assert( symbGetType( sym ) = FB_DATATYPE_FIXUSTR )
	function = ((sym)->lgt \ typeGetSize( FB_DATATYPE_FIXUSTR )) - 1
end function
