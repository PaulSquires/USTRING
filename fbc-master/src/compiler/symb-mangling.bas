...
function hMangleBuiltInType _
	...
	'' same shape as the STRING case above: the leading number is the length of
	'' the identifier that follows, so "FBUSTRING" (9 chars) mangles as 9FBUSTRING
	...
	{ _
		...
		NULL, _ '' var-len ustring (special-cased above, as var-len string is)
		NULL  _ '' fix-len ustring (never reaches here, as fix-len string does not)
	}
