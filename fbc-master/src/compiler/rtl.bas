	...
	case FB_DATATYPE_FIXUSTR
		...
		if( s = NULL ) then
			...
			if( symbGetType( s ) <> typeGetDtAndPtrOnly( dtype ) ) then
				...
				'' USTRING * N carries its length in the FB_STRISFIXED high bit,
				'' as STRING * N does above. The value is the count of usable
				'' code units, EXCLUDING the terminator.
				if( fbIs64bit() ) then
					function = symbGetUstrLength( s ) or &h8000000000000000ull
				else
					function = symbGetUstrLength( s ) or &h80000000ul
				end if
			end if
		end if

	case else
		'' var-len STRING or USTRING -- the descriptor sentinel
		function = -1
