...
{ _
	...
	NULL        , _ '' pointer
	NULL        , _ '' xmmword (was implicit before ustring extended this table)
	@"FBUSTRING", _ '' ustring
	NULL          _ '' fix-len ustring (emitted as its element type, like fix-len string)
}
...
private function hEmitArrayDecl( byval sym as FBSYMBOL ptr ) as string
	...
	if( symbIsRef( sym ) = FALSE ) then
		...
		case FB_DATATYPE_FIXUSTR
			'' as WCHAR: the declared count includes the terminator
			length = symbGetUstrLength( sym ) + 1
		end select
		if( length > 0 ) then
			s += "[" + str( length ) + "]"
		end if
...
private function _emitBegin( ) as integer
	...
	if( fbIs64bit( ) ) then
		...
		'' A ustring code unit is uint16 on EVERY target -- deliberately not
		'' wchar_t, which is 2 bytes on Windows and 4 on Linux. Must stay in
		'' step with FBUSTRING in src/rtlib/fb_ustring.h.
		hWriteLine( "typedef struct { uint16 *data; int64 len; int64 size; } FBUSTRING;", TRUE )
	else
		hWriteLine( "typedef struct { char *data; int32 len; int32 size; } FBSTRING;", TRUE )
		hWriteLine( "typedef struct { uint16 *data; int32 len; int32 size; } FBUSTRING;", TRUE )
	end if
...
private function hEmitType _
	...
	case FB_DATATYPE_FIXUSTR
		'' a code unit is uint16 on every target, so unlike WCHAR above this
		'' needs no per-target remap at all
		s = *dtypeName(FB_DATATYPE_USHORT)
