...
function cSymbolType _
	...
	if( lexGetToken( ) = FB_TK_TYPEOF ) then
		...
		case FB_TK_USTRING
			...
			'' assume var-len, as FB_TK_STRING does; the '* N' branch below
			'' rewrites this to FB_DATATYPE_FIXUSTR when a length is given
	...
	if( lexGetToken( ) = CHAR_TIMES ) then
		...
		case FB_DATATYPE_USTRING
			...
			'' remap type, as STRING -> FIXSTR above
			dtype = FB_DATATYPE_FIXUSTR

			'' as WSTRING * N: lgt is stored in BYTES, and the declared count
			'' includes the terminator. Use symbGetUstrLength() to get the
			'' length back in code units.
			lgt *= typeGetSize( FB_DATATYPE_FIXUSTR )
